...
assert.equal(ontology.inventory.owner_total,41);
assert.equal(ontology.inventory.research_programmes,40);
...
assert.equal(d.repos.length,40);
...
assert.equal(model.syndromeAssembly.renderedRoutes,false);
...
console.log('Tower contract: 40 entrances, 5 motifs, 20 typed contacts; state-resolution and syndrome-assembly remain architectural non-edge syntheses.');
