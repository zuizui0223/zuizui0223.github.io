(function(){
  ...
  // Keep the cover philosophy synchronized without rewriting the static shell.
  ...
  function patchEvolve(){
    ...
    if(!target)return;
    // Canonical safe conceptual direction: EGWE relational state -> PAYOFF context.
    // Reverse architecture -> relational-state direction remains the unresolved closure condition.
  ...
  const observer=new MutationObserver(()=>patchEvolve());
  observer.observe(svg,{childList:true,subtree:true});
  stage.querySelectorAll('.world-lens-btn').forEach(btn=>btn.addEventListener('click',()=>queueMicrotask(patchEvolve)));
  patchEvolve();
})();
